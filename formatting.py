...
def format_trade_alert(e: dict, wallet: Wallet, tier: tuple[str, str],
    ...
    """Telegram push for one new trade on a watched wallet.

    ``e`` may be a single fill or a merged cluster (``count`` > 1), in which case
    the amount/value are totals and ``price_usd`` is the notional-weighted average.
    """
    ...
    if count > 1:  # 合并后的价格是名义金额加权均价
        price_str = f" @ 均价 {_price2(price)}" if price else ""
    lev = e.get("leverage")
    lev_tag = f" · {lev}x" if lev else ""
    merge_tag = f" · 合并 {count} 笔" if count > 1 else ""
    ...
    lines = [
        ...
        f"{_fmt2(amount)}（≈ {_usd2(value)}{price_str}）· {venue_tag}{lev_tag}{merge_tag}",
    ]
    if pnl:
        sign = "+" if pnl >= 0 else ""
