...
在面板上亮起**红圈未读提醒**，同时可对指定钱包**定向推送 Telegram**。
另附一个 **Arkham 风格的深色网页面板**。
...
| 定向 TG 推送 | 只推名称以 `PUSH_PREFIX`（默认 `ZR`）开头的钱包；短时间内连续的相同动作自动合并成一条 |
...
- **定向推送**：名称以 `PUSH_PREFIX`（默认 `ZR`）开头的钱包会额外推送到 Telegram，
  其余钱包只亮红圈不打扰；`PUSH_MERGE_WINDOW`（默认 300s）内同币种同方向的连续成交
  会合并成一条（HL 常把大单拆成几十笔，不合并会刷屏）。推送走 Bot HTTP API，
  `BOT_ENABLED=false` 也能发。
